//! Data structures for Guitar Hero World Tour.
//!
...
impl MatchDetails for GHWTMatchDetails {
...
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema)]
pub struct GHWTPerformanceDetails {
    /// Played instrument.
...
#[typetag::serde(name = "ghwt")]
impl PerformanceDetails for GHWTPerformanceDetails {
    fn sorting_key(&self) -> f64 {
        self.score as f64
    }
...
pub struct GuitarHeroWorldTour;

#[typetag::serde(name = "ghwt")]
impl Game for GuitarHeroWorldTour {
    ...
    fn create_match_and_performance_from_spreadsheet_record(&self, record: &Record, _ctx: &mut Context) -> ParseMatchRecordResult {
        ...
        let match_data = GHWTMatchDetails {
            mode: Mode::UnknownSingle,
            game_version: None,
        };
        let performance_data = GHWTPerformanceDetails {
    ...
    game_impl!(GHWTMatchDetails, GHWTPerformanceDetails);
}

register_game!(GuitarHeroWorldTour);
